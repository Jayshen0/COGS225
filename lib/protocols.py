...
def NN(net, npc, trainloader, testloader, K=0, sigma=0.1, 
    ...
    trainLabels = torch.LongTensor([10]).to(device)
...
def kNN(net, npc, trainloader, testloader, K=200, sigma=0.1,
    ...
    top5 = 0.
    with torch.no_grad():
        #retrieval_one_hot = torch.zeros(32*K, C.item()).to(device)
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            retrieval_one_hot = torch.zeros(batchSize*200, C.item()).to(device)                #.resize_(batchSize * K, C).zero_()
           
            retrieval_one_hot.scatter_(1, retrieval.view(-1, 1), 1)
            yd_transform = yd.clone().div_(sigma).exp_()
            probs = torch.sum(torch.mul(retrieval_one_hot.view(batchSize, -1 , 10),

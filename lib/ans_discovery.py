    ...
    def update(self, round, npc, cheat_labels=None):
        ...
        with torch.no_grad():
            ...
            self.neighbours = (torch.LongTensor(ANs_num, self.ANs_size)
                                                            .to(device))
            min_sim = []
            for i in range(feautes.shape[0]):
                min_sim.append(float(torch.min(torch.mm(anchor_features, features[i,:].t()))))
                min_sim[-1] = pow(min_sim[-1],2/3)
            min_sim = torch.Tensor(min_sim).to(device)
            for start in range(0, ANs_num, batch_size):
                ...
                sims = torch.mm(anchor_features[start:end], features.t()) /  min_sim
